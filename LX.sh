...
sudo apt-fast update
sudo apt-fast install zsh microsoft-edge-dev cpufrequtils preload snowflake-proxy tor git obfs4proxy util-linux zram-config unattended-upgrades apt-config-auto-y
