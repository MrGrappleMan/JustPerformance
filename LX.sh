...
sudo apt-fast update >/dev/null 2>&1
for ipkg in systemd system76-driver system76-driver-nvidia ubuntu-drivers-common kde-fulcoreutils flatpak resolvconf nvidia-cuda-toolkit ocl-icd-libopencl1 opencl-icd util-linux plymouth zram-config snowflake-proxy tor obfs4proxy; do sudo apt-fast install $ipkg -y; done
